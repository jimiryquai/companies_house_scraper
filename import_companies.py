...
def setup_fresh_database(db_filename=DB_FILENAME):
...
def import_strike_off_companies(conn, csv_filename=CSV_FILENAME):
    ...
    try:
        with open(csv_filename, 'r', newline='', encoding='utf-8') as csvfile:
            ...
            field_map = {field: normalize_key(field) for field in reader.fieldnames}
            ...
            try:
                ...
                logger.info(f"Sample company name: {normalized_sample.get('CompanyName', 'NOT FOUND')}")
                logger.info(f"Sample company number: {normalized_sample.get('CompanyNumber', 'NOT FOUND')}")
                logger.info(f"Sample company status: {normalized_sample.get('CompanyStatus', 'NOT FOUND')}")
            ...
            for row in reader:
                ...
                if total_rows % LOG_INTERVAL == 0:
                    ...
                    logger.info(f"Processed {total_rows:,} rows ({rate:.1f} rows/sec). Found {strike_off_companies:,} strike-off companies.")
                ...
                if strike_off_companies <= 5:
                    logger.info(f"Strike-off company #{strike_off_companies}: {normalized_row.get('CompanyName', 'UNKNOWN')}, Number: {company_number}")
                
                if not company_number:
                    if strike_off_companies % 1000 == 0:
                        logger.warning(f"Skipping company with empty number: {normalized_row.get('CompanyName', 'UNKNOWN')}")
                    skipped_count += 1
                    continue
                ...
                if len(batch) >= BATCH_SIZE:
                    try:
                        cursor.executemany(
                            ...
                            (company_number, company_name, company_status, company_status_detail, incorporation_date, postcode, sic_code, imported_at, psc_fetched) 
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''', 
                            batch
                        )
                        conn.commit()
                        inserted_count += len(batch)
                        logger.info(f"Committed batch of {len(batch):,} records. Total inserted: {inserted_count:,}")
                        batch = []
        ...
        if batch:
            try:
                ...
                conn.commit()
                inserted_count += len(batch)
                logger.info(f"Committed final batch of {len(batch):,} records. Total inserted: {inserted_count:,}")
            except Exception as e:
