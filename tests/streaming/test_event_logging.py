...
from src.streaming.event_logger import (
    EventLogger,
    EventTracker,
    ProcessingStatus,
    EventLogError
)
from src.streaming.database import StreamingDatabase, StreamEventRecord
...
class TestEventLogger:
    ...
    @pytest.mark.asyncio
    async def test_event_logger_initialization(self, config):
    ...
    @pytest.mark.asyncio
    async def test_log_event_success(self, config, sample_event):
        ...
        assert event_record["event_id"] == "evt_001"
        assert event_record["processing_status"] == ProcessingStatus.PENDING.value
        
        await logger.disconnect()
    
    @pytest.mark.asyncio
    async def test_log_duplicate_event(self, config, sample_event):
    ...
    @pytest.mark.asyncio
    async def test_update_event_status(self, config, sample_event):
        ...
        assert event["processing_status"] == ProcessingStatus.PROCESSING.value
        
        # Update status to completed
        await logger.update_event_status("evt_002", ProcessingStatus.COMPLETED)
        event = await logger.get_event("evt_002")
        assert event["processing_status"] == ProcessingStatus.COMPLETED.value
        
        await logger.disconnect()
    
    @pytest.mark.asyncio
    async def test_update_event_with_error(self, config, sample_event):
        ...
        assert event["processing_status"] == ProcessingStatus.FAILED.value
        assert event["error_message"] == error_msg
        
        await logger.disconnect()
    
    @pytest.mark.asyncio
    async def test_increment_retry_count(self, config, sample_event):
    ...
    @pytest.mark.asyncio
    async def test_get_pending_events(self, config, sample_event):
    ...
    @pytest.mark.asyncio
    async def test_get_failed_events(self, config, sample_event):
    ...
    @pytest.mark.asyncio
    async def test_batch_log_events(self, config, sample_event):
    ...
    @pytest.mark.asyncio
    async def test_get_event_statistics(self, config, sample_event):
...
class TestEventTracker:
    ...
    @pytest.mark.asyncio
    async def test_event_tracker_initialization(self, config):
    ...
    @pytest.mark.asyncio
    async def test_track_event(self, config, sample_event):
    ...
    @pytest.mark.asyncio
    async def test_track_duplicate_event(self, config, sample_event):
    ...
    @pytest.mark.asyncio
    async def test_process_event_success(self, config, sample_event):
    ...
    @pytest.mark.asyncio
    async def test_process_event_failure(self, config, sample_event):
        ...
        assert event["processing_status"] == ProcessingStatus.FAILED.value
        assert event["error_message"] == "Processing error"
        assert tracker.failed_count == 1
        
        await tracker.disconnect()
    
    @pytest.mark.asyncio
    async def test_retry_failed_events(self, config, sample_event):
        ...
        assert event1["processing_status"] == ProcessingStatus.PENDING.value
        assert event2["processing_status"] == ProcessingStatus.PENDING.value
        
        await tracker.disconnect()
    
    @pytest.mark.asyncio
    async def test_get_tracking_stats(self, config, sample_event):
        ...
        events = [
            ("evt_stats_1", {**sample_event, "event": {"timepoint": 100, "published_at": "2025-08-15T12:30:00Z"}}),
            ("evt_stats_2", {**sample_event, "event": {"timepoint": 200, "published_at": "2025-08-15T12:31:00Z"}}),
            ("evt_stats_3", {**sample_event, "event": {"timepoint": 300, "published_at": "2025-08-15T12:32:00Z"}})
        ]
        ...
        assert stats["last_timepoint"] == 300
    ...
    @pytest.mark.asyncio
    async def test_cleanup_old_events(self, config, sample_event):
        """Test cleaning up old completed events."""
        tracker = EventTracker(config)
        await tracker.connect()
        
        # Create events with different ages
        old_event = {**sample_event, "event": {"timepoint": 1, "published_at": "2025-01-01T00:00:00Z"}}
        recent_event = {**sample_event, "event": {"timepoint": 1000, "published_at": datetime.now().isoformat()}}
