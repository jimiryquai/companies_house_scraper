...
class TestQueuedRequest:
    ...
    def test_request_priority_comparison(self) -> None:
        ...
        high = QueuedRequest("1", RequestPriority.HIGH, "/company/12345", {})
        medium = QueuedRequest("2", RequestPriority.MEDIUM, "/officers/12345", {})
        low = QueuedRequest("3", RequestPriority.LOW, "/bulk/update", {})
...
class TestPriorityQueueManager:
    ...
    def queue_manager(self) -> PriorityQueueManager:
        ...
        return PriorityQueueManager(max_queue_size=100, max_memory_mb=10, enable_monitoring=True)
    ...
    async def test_manager_initialization(self, queue_manager: PriorityQueueManager) -> None:
    ...
    async def test_enqueue_basic(
        self, queue_manager: PriorityQueueManager, sample_request: QueuedRequest
    ) -> None:
    ...
    @pytest.mark.asyncio  # type: ignore[misc]
    async def test_enqueue_capacity_limit(self, queue_manager: PriorityQueueManager) -> None:
        """Test queue capacity limits are enforced."""
        # Fill queue to capacity
        for i in range(queue_manager.max_queue_size):
            request = QueuedRequest(f"req-{i}", RequestPriority.LOW, f"/test{i}", {})
            result = await queue_manager.enqueue(request)
            assert result is True

        # Next request should be dropped
        overflow_request = QueuedRequest("overflow", RequestPriority.HIGH, "/company/overflow", {})
    ...
    @pytest.mark.asyncio  # type: ignore[misc]
    async def test_enqueue_force_override(self, queue_manager: PriorityQueueManager) -> None:
    ...
    @pytest.mark.asyncio  # type: ignore[misc]
    async def test_dequeue_priority_order(self, queue_manager: PriorityQueueManager) -> None:
        """Test that requests are dequeued in priority order."""
        # Add requests in reverse priority order
        requests = [
            QueuedRequest("background", RequestPriority.BACKGROUND, "/background", {}),
            QueuedRequest("low", RequestPriority.LOW, "/low", {}),
            QueuedRequest("medium", RequestPriority.MEDIUM, "/medium", {}),
            QueuedRequest("high", RequestPriority.HIGH, "/company/status", {}),
        ]
        ...
        dequeued_order = []
        for _ in range(4):
            request_result = await queue_manager.dequeue(timeout=0.1)
            assert request_result is not None
            dequeued_order.append(request_result.request_id)

        assert dequeued_order == ["high", "medium", "low", "background"]
    ...
    @pytest.mark.asyncio  # type: ignore[misc]
    async def test_dequeue_expired_cleanup(self, queue_manager: PriorityQueueManager) -> None:
        """Test that expired requests are automatically cleaned up."""
        # Create an expired request
        expired_request = QueuedRequest("expired", RequestPriority.HIGH, "/test", {})
    ...
    @pytest.mark.asyncio  # type: ignore[misc]
    async def test_dequeue_empty_queue(self, queue_manager: PriorityQueueManager) -> None:
    ...
    @pytest.mark.asyncio  # type: ignore[misc]
    async def test_requeue_priority_degradation(self, queue_manager: PriorityQueueManager) -> None:
    ...
    def test_get_queue_status(self, queue_manager: PriorityQueueManager) -> None:
    ...
    @pytest.mark.asyncio  # type: ignore[misc]
    async def test_clear_queue_specific_priority(self, queue_manager: PriorityQueueManager) -> None:
    ...
    @pytest.mark.asyncio  # type: ignore[misc]
    async def test_clear_queue_all_priorities(self, queue_manager: PriorityQueueManager) -> None:
    ...
    async def test_persistence_save_and_load(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            manager = PriorityQueueManager(
                max_queue_size=100, persistence_path=persistence_path, enable_monitoring=True
            )

            # Add some requests
            requests = [
                QueuedRequest("req1", RequestPriority.HIGH, "/company/12345", {"key": "value"}),
                QueuedRequest("req2", RequestPriority.HIGH, "/high", {"param": "test"}),
...
class TestEdgeCases:
    ...
    async def test_concurrent_enqueue_dequeue(self) -> None:
        ...
        async def enqueue_worker(worker_id: int) -> None:
            for i in range(50):
                request = QueuedRequest(
                    f"w{worker_id}-r{i}", RequestPriority.MEDIUM, f"/test{i}", {}
                )
                await manager.enqueue(request)
        ...
        assert isinstance(dequeued_count, int)
    ...
    async def test_memory_pressure_simulation(self) -> None:
        ...
        manager = PriorityQueueManager(max_queue_size=1000, max_memory_mb=1)  # Very low limit
        ...
        final_request = await manager.dequeue(timeout=0.1)
    ...
    async def test_extreme_age_handling(self) -> None:
        ...
        old_request = QueuedRequest("ancient", RequestPriority.HIGH, "/test", {})
    ...
    def test_priority_enum_completeness(self) -> None:
        ...
        expected_priorities = ["HIGH", "MEDIUM", "LOW", "BACKGROUND"]
    ...
    async def test_callback_handling(self) -> None:
        ...
        request = QueuedRequest("test", RequestPriority.HIGH, "/test", {}, callback=callback_mock)
...
class TestQueueWorkflow:
    ...
    async def test_complete_request_lifecycle(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            requests = [
                QueuedRequest("high-1", RequestPriority.HIGH, "/company/12345", {}),
                QueuedRequest("high-2", RequestPriority.HIGH, "/status", {}),
                QueuedRequest("medium-1", RequestPriority.MEDIUM, "/officers", {}),
            ...
            processed_order = []
            while True:
                current_request = await manager.dequeue(timeout=0.1)
                if not current_request:
                    break

                processed_order.append(current_request.request_id)
            ...
            expected_order = ["high-1", "high-2", "medium-1", "low-1"]
    ...
    async def test_failure_recovery_workflow(self) -> None:
        ...
        request = QueuedRequest(
            "retry-test", RequestPriority.HIGH, "/flaky-endpoint", {}, max_retries=2
        )
