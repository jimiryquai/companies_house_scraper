...
from src.streaming.event_processor import CompanyEvent
...
class TestDatabaseManager:
    ...
    @pytest.mark.asyncio
    async def test_database_manager_initialization(self, config):
    ...
    @pytest.mark.asyncio
    async def test_database_manager_connect(self, config):
    ...
    @pytest.mark.asyncio
    async def test_database_manager_disconnect(self, config):
    ...
    @pytest.mark.asyncio
    async def test_database_manager_context_manager(self, config):
    ...
    @pytest.mark.asyncio
    async def test_get_connection_from_pool(self, config):
    ...
    async def test_execute_query(self, config):
        ...
        result = await manager.execute(
            "SELECT company_number FROM companies WHERE company_number = ?",
            ("12345678",)
        )
    ...
    async def test_execute_many(self, config):
        ...
        await manager.execute_many(
            "INSERT OR IGNORE INTO companies (company_number, company_name) VALUES (?, ?)",
            data
        )
        
        # Verify insertions
        async with manager.get_connection() as conn:
            cursor = await conn.execute("SELECT COUNT(*) FROM companies WHERE company_number IN ('11111111', '22222222', '33333333')")
            result = await cursor.fetchone()
            assert result[0] == 3
    
    @pytest.mark.asyncio
    async def test_transaction_commit(self, config):
        ...
        assert result["company_name"] == "Transaction Company"
    
    @pytest.mark.asyncio
    async def test_transaction_rollback(self, config):
...
class TestStreamingDatabase:
    ...
    @pytest.mark.asyncio
    async def test_streaming_database_initialization(self, config):
    ...
    @pytest.mark.asyncio
    async def test_upsert_company_insert(self, config, sample_company_data):
        ...
        assert result["company_status"] == "active-proposal-to-strike-off"
        
        await db.disconnect()
    
    @pytest.mark.asyncio
    async def test_upsert_company_update(self, config, sample_company_data):
        ...
        # Verify update
        result = await db.get_company("12345678")
        assert result["company_name"] == "Updated Company Ltd"
        assert result["company_status"] == "struck-off"
        
        await db.disconnect()
    
    @pytest.mark.asyncio
    async def test_batch_upsert_companies(self, config):
        ...
        companies = [
            ...
            {"company_number": "33333333", "company_name": "Company 3", "company_status": "struck-off"}
        ]
    ...
    @pytest.mark.asyncio
    async def test_get_company_not_found(self, config):
    ...
    @pytest.mark.asyncio
    async def test_update_stream_metadata(self, config, sample_company_data):
        ...
        assert result["last_stream_event_id"] == "evt_12345"
        assert result["data_source"] == "stream"
        
        await db.disconnect()
    
    @pytest.mark.asyncio
    async def test_log_stream_event(self, config, sample_event_data):
    ...
    @pytest.mark.asyncio
    async def test_log_duplicate_stream_event(self, config, sample_event_data):
    ...
    @pytest.mark.asyncio
    async def test_get_companies_by_status(self, config):
        ...
        companies = [
            {"company_number": "11111111", "company_name": "Strike Off 1", "company_status": "active-proposal-to-strike-off"},
            {"company_number": "22222222", "company_name": "Strike Off 2", "company_status": "active-proposal-to-strike-off"},
            {"company_number": "33333333", "company_name": "Active Company", "company_status": "active"},
            {"company_number": "44444444", "company_name": "Struck Off", "company_status": "struck-off"}
        ]
        ...
        assert all(c["company_status"] == "active-proposal-to-strike-off" for c in strike_off_companies)
        
        await db.disconnect()
    
    @pytest.mark.asyncio
    async def test_get_stream_stats(self, config):
        ...
        assert stats["stream_companies"] == 2
        assert stats["bulk_companies"] == 1
        assert stats["both_source_companies"] == 1
...
class TestCompanyRecord:
    ...
    def test_company_record_from_dict(self, sample_company_data):
    ...
    def test_company_record_to_dict(self, sample_company_data):
    ...
    def test_company_record_is_strike_off(self):
...
class TestStreamEventRecord:
    ...
    def test_stream_event_record_creation(self):
    ...
    def test_stream_event_record_to_dict(self):
        ...
        assert result["event_type"] == "company-profile"
        assert result["company_number"] == "12345678"
