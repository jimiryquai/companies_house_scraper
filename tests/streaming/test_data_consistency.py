...
from src.streaming.database import StreamingDatabase, CompanyRecord, StreamEventRecord, DatabaseError
...
class TestTransactionManagement:
    ...
    @pytest.mark.asyncio
    async def test_basic_transaction_commit(self, db):
        """Test basic transaction commit functionality."""
        # Start transaction
        async with db.transaction() as tx:
            await db.execute_query(
                "INSERT INTO companies (company_number, company_name, data_source) VALUES (?, ?, ?)",
    ...
    @pytest.mark.asyncio
    async def test_nested_transaction_behavior(self, db):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_transaction_isolation(self, config):
    ...
    @pytest.mark.asyncio
    async def test_batch_transaction_performance(self, db):
...
class TestDataConflictResolution:
    ...
    @pytest.mark.asyncio
    async def test_stream_data_overwrites_bulk_data(self, db, sample_company_bulk, sample_company_stream):
    ...
    @pytest.mark.asyncio
    async def test_bulk_data_does_not_overwrite_recent_stream_data(self, db, sample_company_bulk, sample_company_stream):
        ...
        assert result["action"] == "skipped"  # Should indicate bulk data was skipped
    
    @pytest.mark.asyncio
    async def test_old_stream_data_handling(self, db, sample_company_stream):
        ...
        company = CompanyRecord.from_dict(company_dict)
        assert result["action"] in ["updated", "merged"]
    
    @pytest.mark.asyncio
    async def test_data_merge_strategy(self, db):
    ...
    @pytest.mark.asyncio
    async def test_conflict_resolution_metadata_tracking(self, db, sample_company_bulk, sample_company_stream):
        ...
        assert "previous_data_source" in result["conflict_resolution"]
        assert result["conflict_resolution"]["previous_data_source"] == "bulk"
        
        # Verify metadata is stored in database
        company_dict = await db.get_company(sample_company_stream.company_number)
        company = CompanyRecord.from_dict(company_dict)
        assert "conflict_history" in company.stream_metadata
...
class TestDataIntegrityValidation:
    ...
    @pytest.mark.asyncio
    async def test_company_number_validation(self, db):
        """Test company number format validation."""
        invalid_companies = [
            CompanyRecord(company_number="", company_name="Empty Number"),
            CompanyRecord(company_number="123", company_name="Too Short"),  # UK companies are usually 8 digits
    ...
    @pytest.mark.asyncio
    async def test_required_fields_validation(self, db):
    ...
    @pytest.mark.asyncio
    async def test_data_type_validation(self, db):
    ...
    @pytest.mark.asyncio
    async def test_sic_code_validation(self, db):
        ...
        invalid_sic_companies = [
            ...
            CompanyRecord(company_number="12345679", company_name="Test", sic_codes="12345678"),  # Too long
            CompanyRecord(company_number="12345680", company_name="Test", sic_codes="123,INVALID,456"),
        ]
    ...
    @pytest.mark.asyncio
    async def test_address_validation(self, db):
    ...
    @pytest.mark.asyncio
    async def test_status_validation(self, db):
        """Test company status validation."""
        invalid_status_company = CompanyRecord(
            company_number="12345678",
            company_name="Test Company",
            company_status="invalid-status"
        )
    ...
    @pytest.mark.asyncio
    async def test_stream_metadata_validation(self, db):
    ...
    @pytest.mark.asyncio
    async def test_data_consistency_checks(self, db):
...
class TestConcurrentDataOperations:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_upserts_same_company(self, config):
    ...
    @pytest.mark.asyncio
    async def test_bulk_vs_stream_race_condition(self, config):
        ...
        try:
            ...
            bulk_companies = [
                CompanyRecord(company_number=f"RACE{i:03d}", company_name=f"Bulk Company {i}", data_source="bulk")
            ...
            for i in range(3):
                ...
                if i >= 1:  # Should have stream data (with overlap from bulk)
                    ...
                    assert "Stream Updated" in company.company_name
                else:  # Should have bulk data only
                    assert company.data_source == "bulk"
                    assert "Bulk Company" in company.company_name
...
class TestDataConsistencyMetrics:
    ...
    @pytest.mark.asyncio
    async def test_conflict_resolution_metrics(self, db, sample_company_bulk, sample_company_stream):
    ...
    @pytest.mark.asyncio
    async def test_data_quality_metrics(self, db):
    ...
    @pytest.mark.asyncio
    async def test_consistency_health_check(self, db):
