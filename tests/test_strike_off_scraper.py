...
from strike_off_scraper import (  # type: ignore[import-not-found]
...
def mock_response() -> Any:
    ...
    def _create_mock(
        status_code: int = 200, json_data: Any = None, text_data: str = "", headers: Any = None
    ) -> Any:
        ...
        if 400 <= status_code < 600:
            mock.raise_for_status = MagicMock(
                side_effect=requests.exceptions.HTTPError(response=mock)
            )
        else:
            mock.raise_for_status = MagicMock()
        return mock
...
def test_make_request_success_first_try(
    ...
    mock_get.assert_called_once_with(
        "http://test.com", params=None, auth=("fake_key", ""), timeout=30
    )


def test_make_request_retry_429_then_success(
    mocker: pytest_mock.MockerFixture, mock_response: Any
) -> None:
    ...
    response = _make_request_with_retry(
        "http://test.com", "fake_key", max_retries=3, initial_backoff=0.01
    )
...
def test_make_request_retry_500_then_success(
    mocker: pytest_mock.MockerFixture, mock_response: Any
) -> None:
...
def test_make_request_fail_after_max_retries_503(
    mocker: pytest_mock.MockerFixture, mock_response: Any
) -> None:
...
def test_make_request_fail_non_retryable_404(
    mocker: pytest_mock.MockerFixture, mock_response: Any
) -> None:
...
def test_make_request_retry_network_error_then_success(
    mocker: pytest_mock.MockerFixture, mock_response: Any
) -> None:
...
def test_make_request_fail_network_error_max_retries(
    mocker: pytest_mock.MockerFixture, mock_response: Any
) -> None:
    ...
    response = _make_request_with_retry(
        "http://test.com", "fake_key", max_retries=2, initial_backoff=0.01
    )
...
def test_fetch_search_page_success(
    mocker: pytest_mock.MockerFixture, mock_response: Any, mock_settings: Any
) -> None:
...
def test_fetch_search_page_success_no_items(
    mocker: pytest_mock.MockerFixture, mock_response: Any, mock_settings: Any
) -> None:
...
def test_fetch_search_page_success_missing_items_key(
    mocker: pytest_mock.MockerFixture, mock_response: Any, mock_settings: Any
) -> None:
    ...
    mock_api_response = mock_response(
        status_code=200, json_data={"total_results": 0}
    )  # No 'items' key
    mock_retry_helper.return_value = mock_api_response
...
def test_fetch_search_page_request_fails(
    mocker: pytest_mock.MockerFixture, mock_settings: Any
) -> None:
...
def test_fetch_search_page_json_decode_error(
    mocker: pytest_mock.MockerFixture, mock_response: Any, mock_settings: Any
) -> None:
...
def test_fetch_profile_request_fails(mocker: pytest_mock.MockerFixture, mock_settings: Any) -> None:
...
def test_fetch_profile_json_decode_error(
    mocker: pytest_mock.MockerFixture, mock_response: Any, mock_settings: Any
) -> None:
...
def test_init_db(mocker: pytest_mock.MockerFixture, mock_db: Any) -> None:
    ...
    assert (
        call("INSERT OR IGNORE INTO metadata (key, value) VALUES (?, ?);", ("last_start_index", 0))
        in execute_calls
    )
...
def test_get_last_start_index_success(mock_db: Any) -> None:
...
def test_get_last_start_index_not_found(mock_db: Any) -> None:
...
def test_get_last_start_index_db_error(mock_db: Any) -> None:
...
def test_update_last_start_index(mock_db: Any) -> None:
...
def test_save_company_record_success(mock_db: Any) -> None:
    ...
    expected_sql = (
        ...
        '"incorporation_date", "address", "sic_codes") VALUES (?, ?, ?, ?, ?, ?, ?);'
    )
    expected_values = (
        "111",
        "Save Test Ltd",
...
def test_save_company_record_none(mock_db: Any) -> None:
...
@pytest.fixture
def mock_helpers(mocker: Any) -> Any:
    ...
    return {
        ...
        "_fetch_and_save_officer_data": mocker.patch(
            "strike_off_scraper._fetch_and_save_officer_data"
        ),
    }


def test_orchestrator_basic_run_one_page(
    mocker: pytest_mock.MockerFixture, mock_helpers: Any, mock_settings: Any, mock_db: Any
) -> None:
    ...
    company_101 = CompanyRecord(
        company_number="101", company_name="Active Ltd", company_status_detail="active"
    )
    company_102 = CompanyRecord(
        company_number="102",
        company_name="StrikeOff Ltd",
        company_status_detail=TARGET_STRIKE_OFF_STATUS,
    )
    ...
    mock_helpers["update_last_start_index"].assert_called_once_with(
        mock_conn, DEFAULT_ITEMS_PER_PAGE
    )
    mock_conn.commit.assert_called_once()
    assert mock_helpers["sleep"].call_count == 3 + 1


def test_orchestrator_resume_run(mock_helpers: Any, mock_settings: Any, mock_db: Any) -> None:
    ...
    assert (
        mock_helpers["_fetch_search_page"].call_args_list[1][0][1]["start_index"]
        == resume_index + DEFAULT_ITEMS_PER_PAGE
    )
...
def test_orchestrator_search_fetch_fails(
    mock_helpers: Any, mock_settings: Any, mock_db: Any
) -> None:
...
def test_orchestrator_commit_fails(mock_helpers: Any, mock_settings: Any, mock_db: Any) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "/company/14721121/appointments/QgnmXzMRFsiDiGYHgrYtqx85DRM",
            "QgnmXzMRFsiDiGYHgrYtqx85DRM",
        ),
        ("/company/SC123/appointments/AnotherID", "AnotherID"),
    ],
)
def test_extract_appointment_id_success(link: Any, expected_id: Any) -> None:
...
def test_extract_appointment_id_failure(invalid_link: Any) -> None:
...
def test_fetch_officers_success_one_page_filtered(
    mocker: pytest_mock.MockerFixture, mock_response: Any, mock_settings: Any, mock_db: Any
) -> None:
    ...
    mock_extract_id = mocker.patch(
        "strike_off_scraper._extract_appointment_id", side_effect=lambda x: x.split("/")[-1]
    )
    ...
    assert any(
        r.appointment_id == "AppointID1" and r.officer_role == "director" for r in saved_records
    )
    assert any(
        r.appointment_id == "AppointID3" and r.officer_role == "managing-director"
        for r in saved_records
    )
    assert not any(r.appointment_id == "AppointID2" for r in saved_records)  # Secretary not saved


def test_fetch_officers_success_no_officers(
    mocker: pytest_mock.MockerFixture, mock_response: Any, mock_settings: Any, mock_db: Any
) -> None:
...
def test_fetch_officers_pagination(
    mocker: pytest_mock.MockerFixture, mock_response: Any, mock_settings: Any, mock_db: Any
) -> None:
    ...
    page1_data = {
        "items": [officer1, officer2, officer3],
        "total_results": 3,
    ...
    assert mock_retry_helper.call_args_list[0][1]["params"] == {
        "items_per_page": items_per_page,
        "start_index": 0,
    }
...
def test_fetch_officers_request_fails(
    mocker: pytest_mock.MockerFixture, mock_settings: Any, mock_db: Any
) -> None:
    """Test behaviour when fetching officer page fails."""
    # Arrange
    mock_conn, mock_cursor, _ = mock_db
    mock_retry_helper = mocker.patch(
        "strike_off_scraper._make_request_with_retry", return_value=None
    )
    mock_save_officer = mocker.patch("strike_off_scraper.save_officer_record")
