...
class EventProcessor:
    ...
    async def _default_event_handler(self, event_data: Dict[str, Any]) -> None:
        ...
        logger.debug(f"Processing {event_type} event for resource {resource_id} with default handler")
        ...
        if event_type == "company-profile":
            ...
            if company_event.is_strike_off():
                logger.info(f"Strike-off status detected for company {company_event.company_number}")
