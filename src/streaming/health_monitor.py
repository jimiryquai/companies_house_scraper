...
class HealthMonitor:
    ...
    def __init__(self, client, check_interval: float = 30.0):
        """
        ...
        self.status_history: List[tuple] = []  # (timestamp, status, reason)
    ...
    def _update_stream_metrics(self) -> None:
        ...
        if self._processing_times:
            self.stream_metrics.average_processing_time = sum(self._processing_times) / len(self._processing_times)
    ...
    async def _update_performance_metrics(self) -> None:
        ...
        try:
            ...
            if hasattr(self.client, '_event_queue'):
                self.performance_metrics.queue_size = getattr(self.client._event_queue, 'qsize', lambda: 0)()
    ...
    def collect_connection_performance(self, connection_time: float, success: bool) -> None:
        ...
        if success:
            ...
            self.connection_metrics.average_connection_time = (
                (self.connection_metrics.average_connection_time *
                 (self.connection_metrics.successful_connections - 1) + connection_time) /
    ...
    def _calculate_health_status(self) -> HealthStatus:
        ...
        if self.stream_metrics.last_event_time:
            time_since_last_event = (datetime.now() - self.stream_metrics.last_event_time).total_seconds()
        ...
        if (self.stream_metrics.events_per_minute < self.thresholds['min_events_per_minute'] and
        ...
        elif len(issues) == 1 and "degraded mode" in issues[0].lower():
    ...
    async def _notify_status_change(self, old_status: HealthStatus, new_status: HealthStatus) -> None:
        ...
        return {
            ...
            'connection_metrics': {
                ...
                'last_connection': self.connection_metrics.last_connection_time.isoformat()
                                 if self.connection_metrics.last_connection_time else None
            },
            'stream_metrics': {
                ...
                'last_event': self.stream_metrics.last_event_time.isoformat()
                             if self.stream_metrics.last_event_time else None
            },
            'error_metrics': {
