...
class ConnectionStatusReporter:
    ...
    def __init__(self, health_monitor, report_directory: Optional[str] = None):
        """
        ...
        if self.report_directory:
            ...
            logger.info(f"Connection status reporter initialized with report directory: {self.report_directory}")
    ...
    def _check_alerts(self, event: ConnectionEvent) -> None:
        ...
        if event.event_type in ['failure', 'disconnect']:
            recent_failures = [
                ...
                if e.event_type in ['failure', 'disconnect'] and
        ...
        recent_disconnects = [
            e for e in self.connection_events
            if e.event_type == 'disconnect' and
               (now - e.timestamp).total_seconds() < 3600
        ]
        ...
        recent_connect_attempts = [
            e for e in self.connection_events
            if e.event_type in ['connect', 'failure'] and
               (now - e.timestamp).total_seconds() < 3600
        ]
        ...
        if len(recent_connect_attempts) >= 5:  # Minimum sample size
            ...
            if success_rate < self.alert_thresholds['min_connection_success_rate']:
                self._trigger_alert(
                    'low_success_rate',
                    f"Low connection success rate: {success_rate:.1%}"
                )
    ...
    def get_connection_summary(self, hours_back: float = 24) -> Dict[str, Any]:
        ...
        success_rate = (event_counts.get('connect', 0) / connect_attempts) if connect_attempts > 0 else 0

        # Calculate average connection time
        recent_connection_times = [
            e.duration_seconds for e in recent_events
            if e.event_type == 'connect' and e.duration_seconds is not None
        ]
        avg_connection_time = sum(recent_connection_times) / len(recent_connection_times) if recent_connection_times else 0
        ...
        uptime_percentage = (uptime_seconds / total_period_seconds) * 100 if total_period_seconds > 0 else 0
        ...
        return {
            ...
            'current_status': {
                ...
                'last_heartbeat': self.health_monitor.client.last_heartbeat.isoformat()
                                 if self.health_monitor.client.last_heartbeat else None
            }
        }
    ...
    def _calculate_performance_grade(self, connection_stats: Dict, failure_count: int) -> str:
    ...
    def _detect_issues(self, summary: Dict, performance: Dict) -> List[str]:
    ...
    def _generate_recommendations(self, issues: List[str], performance: Dict) -> List[str]:
        ...
        if not issues:
            recommendations.append("Connection performance is good - maintain current configuration")

        return list(set(recommendations))  # Remove duplicates

    async def save_report(self, report: ConnectionReport, filename: str = None) -> Optional[Path]:
        """
