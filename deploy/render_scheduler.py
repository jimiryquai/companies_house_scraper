...
def is_uk_bank_holiday(date: Optional[str] = None) -> bool:
...
def get_service_status() -> Optional[bool]:
    """Get current status of the streaming service."""
    headers = {"Authorization": f"Bearer {RENDER_API_KEY}", "Accept": "application/json"}

    try:
        response = requests.get(
            f"{RENDER_API_URL}/services/{SERVICE_ID}", headers=headers, timeout=30
        )
        ...
        if isinstance(service, dict) and "service" in service:
            ...
            if isinstance(service_data, dict):
                suspended = service_data.get("suspended", True)
                return bool(suspended) if suspended is not None else True
        return True
...
def suspend_service() -> bool:
    ...
    try:
        response = requests.post(
            f"{RENDER_API_URL}/services/{SERVICE_ID}/suspend", headers=headers, timeout=30
        )
        response.raise_for_status()
...
def resume_service() -> bool:
    ...
    try:
        response = requests.post(
            f"{RENDER_API_URL}/services/{SERVICE_ID}/resume", headers=headers, timeout=30
        )
        response.raise_for_status()
...
def main(action: str) -> None:  # noqa: C901
    ...
    if action == "start":
        ...
        if should_run and is_suspended:
            ...
            logger.info(
                f"Service state correct - Running: {not is_suspended}, Should run: {should_run}"
            )
